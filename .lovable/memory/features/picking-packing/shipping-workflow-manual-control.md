...
## Auto-fetch on mount
The component now auto-fetches the existing label when the order changes (on mount/reopen), so manual labels persist across page reloads.
...
## Unified quote+create flow
The "Crear Guía" button now implements a one-click unified flow: if no quotes are loaded, clicking the button first runs `getQuotesWithRetry` (showing "Cotizando envío..."), auto-selects the best carrier via `selectBestCarrier`, then immediately creates the label (showing "Creando guía..."). The `apiRef.createLabelWithDefaults` also uses this unified flow for external callers. A `quoteAndCreatePhase` state ('idle' | 'quoting' | 'creating') drives the button text. If quotes are already loaded, it falls back to the direct `handleCreateLabel` behavior.

## Auto-tracking
When an active label with a tracking number is detected, the component automatically calls `trackShipment` to display a status badge and collapsible event timeline.
