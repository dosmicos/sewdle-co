      ...
      if (data.user) {
        ...
        try {
          const {
            data: setupData,
            error: setupError
          } = await supabase.functions.invoke('complete-organization-setup', {
            body: {
              userId: data.user.id,
              organizationName: formData.organizationName,
              organizationType: formData.organizationType,
        ...
        const {
          ...
          body: {
            planId: formData.selectedPlan === 'professional' ? 'plan_professional' : formData.selectedPlan === 'enterprise' ? 'plan_enterprise' : 'plan_starter',
            organizationName: formData.organizationName,
            organizationType: formData.organizationType,
            userId: data.user.id,
            userEmail: formData.email,
  ...
  return <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        ...
        <div className="flex items-center justify-center mb-8">
          {[1, 2, 3].map(num => <div key={num} className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step >= num ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-600'}`}>
        ...
        <Card className="p-8">
          ...
          {step === 1 && <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="text" value={formData.name} onChange={e => updateFormData('name', e.target.value)} placeholder="Tu nombre completo" className="h-12" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Correo electrónico *
                  </label>
                  <Input type="email" value={formData.email} onChange={e => updateFormData('email', e.target.value)} placeholder="tu@email.com" className="h-12" />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="password" value={formData.password} onChange={e => updateFormData('password', e.target.value)} placeholder="Mínimo 6 caracteres" className="h-12" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Confirmar contraseña *
                  </label>
                  <Input type="password" value={formData.confirmPassword} onChange={e => updateFormData('confirmPassword', e.target.value)} placeholder="Repite tu contraseña" className="h-12" />
                </div>
              </div>
          ...
          {step === 2 && <div className="space-y-6">
              ...
              <div>
                ...
                <Input type="text" value={formData.organizationName} onChange={e => updateFormData('organizationName', e.target.value)} placeholder="Nombre de tu marca o taller" className="h-12" />
              </div>
              ...
              <div>
                ...
                <RadioGroup value={formData.organizationType} onValueChange={value => updateFormData('organizationType', value)} className="grid md:grid-cols-3 gap-4">
                  {[{
                id: 'brand',
                label: 'Marca de Moda',
                description: 'Diseño y venta de productos'
              }, {
                id: 'other',
                label: 'Otro',
                description: 'Otro tipo de negocio'
              }].map(type => <div key={type.id} className="relative">
                      <RadioGroupItem value={type.id} id={type.id} className="peer sr-only" />
                      <Label htmlFor={type.id} className={`block p-4 cursor-pointer transition-all border rounded-lg text-center peer-checked:border-orange-500 peer-checked:bg-orange-50 hover:border-gray-300 ${formData.organizationType === type.id ? 'border-orange-500 bg-orange-50' : 'border-gray-200'}`}>
          ...
          {step === 3 && <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {plan.features.map((feature, index) => <span key={index} className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-800">
                              <CheckCircle className="w-3 h-3 mr-1 text-green-500" />
                              {feature}
                            </span>)}
                        </div>
                      </div>
                      <div className={`w-4 h-4 rounded-full border-2 ml-4 ${formData.selectedPlan === plan.id ? 'bg-orange-500 border-orange-500' : 'border-gray-300'}`}>
              ...
              <div className="flex gap-4">
                ...
                <Button onClick={handleSignup} disabled={isLoading} className="flex-1 h-12 bg-orange-500 hover:bg-orange-600">
                  {isLoading ? <>
        ...
        <div className="text-center mt-6">
          <p className="text-sm text-gray-600">
            ¿Ya tienes cuenta?{' '}
            <button onClick={() => navigate('/auth')} className="text-orange-500 hover:text-orange-600 font-medium">
              Iniciar sesión
            </button>
          </p>
        </div>
