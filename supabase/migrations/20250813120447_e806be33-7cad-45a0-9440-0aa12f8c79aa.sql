-- Actualizar la función de cálculo de reposición con nuevos valores de lead time y safety days
-- Lead time: 30 días, Safety days: 10 días
...
DECLARE
  ...
  urgency TEXT;
  reason_text TEXT;
  lead_time_days INTEGER := 30; -- Lead time de 30 días
  safety_days INTEGER := 10; -- Días de seguridad
...
BEGIN
  ...
  RAISE NOTICE 'Iniciando cálculo de reposición con lead time: % días, safety: % días (total: % días)', 
    lead_time_days, safety_days, target_stock_days;

  -- Limpiar sugerencias previas del día actual
  DELETE FROM public.replenishment_suggestions 
  WHERE calculation_date = CURRENT_DATE
  AND organization_id = current_org_id;

  -- Iterar sobre todas las variantes activas con ventas
  FOR rec IN 
    SELECT 
      pv.id as variant_id,
  ...
  LOOP
    ...
    IF days_remaining <= (lead_time_days * 0.5) THEN -- Menos de 15 días
      urgency := 'critical';
