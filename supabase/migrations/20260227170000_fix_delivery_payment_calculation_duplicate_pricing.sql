-- Fix calculate_delivery_payment:
-- 1. Use LATERAL subquery to pick only the most recent active workshop_pricing row
--    per product, preventing row duplication when multiple pricing entries exist.
-- 2. Return items_without_workshop_price count so the UI can warn users when
--    fallback (order sale price) is being used instead of workshop pricing.
