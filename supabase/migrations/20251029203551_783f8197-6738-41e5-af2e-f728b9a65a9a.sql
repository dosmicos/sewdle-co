-- ============================================================================
-- FIX: JOIN correcto usando shopify_order_id (bigint) en lugar de id (uuid)
-- ============================================================================
...
BEGIN
  ...
  WITH sales_data AS (
    -- JOIN CORRECTO: usar shopify_order_id (bigint) de ambas tablas
    SELECT 
      soli.sku,
      COUNT(DISTINCT so.shopify_order_id) as orders_count,
      SUM(soli.quantity) as total_sales
    FROM shopify_order_line_items soli
    JOIN shopify_orders so ON soli.shopify_order_id = so.shopify_order_id  -- ✅ bigint = bigint
  ...
  WHERE vm.sales_30d > 0 OR vm.current_stock > 0
  ORDER BY 
    CASE
