...
-- Recreate with corrected sales_data CTE
...
BEGIN
  ...
  open_orders_data AS (
    SELECT 
      oi.product_variant_id,
      COALESCE(SUM(oi.quantity) - COALESCE(dq.total_delivered, 0), 0)::integer as open_quantity
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.id
    LEFT JOIN delivered_quantities dq ON oi.product_variant_id = dq.product_variant_id
    WHERE o.status IN ('pending', 'in_production', 'assigned')
      AND o.organization_id = current_org_id
    GROUP BY oi.product_variant_id, dq.total_delivered
  ),
  inventory_data AS (
    SELECT 
      pv.id as variant_id,
