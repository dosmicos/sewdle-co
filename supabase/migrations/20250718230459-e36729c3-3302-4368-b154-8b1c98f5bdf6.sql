-- Eliminar la función existente y recrearla con la nueva columna
DROP FUNCTION IF EXISTS public.get_replenishment_suggestions_with_details();

-- Recrear la función con la nueva columna pending_production_quantity
...
AS $function$
  SELECT 
    ...
    rs.calculation_date,
    rs.created_at,
    -- NUEVA COLUMNA: Calcular cantidades pendientes de producir (órdenes no aprobadas)
    COALESCE((
