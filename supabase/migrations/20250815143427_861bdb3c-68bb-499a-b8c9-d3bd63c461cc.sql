...
-- Corrige el error "column pv.is_active does not exist" usando el estado del producto
...
BEGIN
  ...
  FOR rec IN 
    ...
    WHERE p.organization_id = current_org_id
      AND (p.status IS NULL OR p.status = 'active')  -- Reemplaza pv.is_active
      AND EXISTS (
        SELECT 1
  ...
  LOOP
    ...
    -- Velocidad de ventas (promedio diario)
    sales_velocity := CASE 
