...
-- Workshop users and other roles will not be able to access customer PII
...
AS $function$
  ...
  WHERE so.organization_id IN (SELECT get_user_organizations())
...
AS $function$
  ...
  WHERE so.created_at_shopify >= start_date 
    ...
    AND so.organization_id IN (SELECT get_user_organizations())
