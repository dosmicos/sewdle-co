-- Update get_material_deliveries_with_real_balance function to ensure 
-- material consumptions without order IDs don't show up
...
AS $$
  WITH material_workshop_totals AS (
    SELECT 
      ...
      -- Balance real = entregado - consumido
  ...
  SELECT DISTINCT ON (md.material_id, md.workshop_id)
    ...
    md.updated_at,
    -- Totales calculados
    mwt.total_delivered_qty::numeric as total_delivered,
    mwt.total_consumed_qty::numeric as total_consumed,
    mwt.real_balance_qty::numeric as real_balance,
