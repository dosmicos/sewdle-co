-- Fix sales velocity to use actual sales data, not workshop deliveries
...
BEGIN
  ...
  LOOP
    -- Calculate sales velocity (units per day) over last 30 days using SALES data, not deliveries
    -- FIXED: Use sales_metrics table which contains actual sales to customers
    SELECT COALESCE(
      SUM(sm.sales_quantity)::NUMERIC / GREATEST(30, 1), 
      0
    ) INTO velocity
