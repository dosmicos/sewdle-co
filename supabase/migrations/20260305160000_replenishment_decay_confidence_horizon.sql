...
-- =====================================================
-- Mejora: Horizonte de proyección por confianza + Decaimiento de velocidad histórica
--
-- Cambios:
-- 1. Nuevo CTE last_stock_date: calcula días sin stock por variante
-- 2. P4 (velocidad histórica) aplica decaimiento basado en días sin stock
-- 3. Horizonte variable: 55d (alta) / 40d (media) / 30d (baja) / 20d (histórica)
-- 4. days_of_supply usa velocidad decaída para P4
-- 5. reason incluye info de horizonte y % decaimiento
-- =====================================================
...
BEGIN
  ...
  -- NUEVO: Último día con stock para calcular decaimiento
  last_stock_date AS (
    SELECT
      psh.product_variant_id as variant_id,
      MAX(DATE(psh.recorded_at)) as last_date_with_stock,
      (CURRENT_DATE - MAX(DATE(psh.recorded_at)))::integer as days_since_last_stock
    FROM product_stock_history psh
    JOIN product_variants pv ON psh.product_variant_id = pv.id
    JOIN products p ON pv.product_id = p.id
    WHERE p.organization_id = org_id
      AND psh.stock_quantity > 0
    GROUP BY psh.product_variant_id
  ),
  ...
  replenishment_calc AS (
    SELECT
      ...
      -- Velocidad ajustada con decaimiento para P4
      CASE
      ...
      -- Días de suministro con velocidad decaída para P4
      CASE
      ...
      -- Proyección con horizonte variable según confianza
      CASE
        -- P1 Alta confianza: ≥15d con stock en 30d → horizonte 55d
        WHEN COALESCE(stk.days_with_stock_30d, 0) >= 15 AND COALESCE(sales.sales_30d, 0) > 0
        THEN ROUND((sales.sales_30d::numeric / stk.days_with_stock_30d) * 55, 0)
        -- P1 Media confianza: 5-14d con stock en 30d → horizonte 40d
        WHEN COALESCE(stk.days_with_stock_30d, 0) >= 5 AND COALESCE(sales.sales_30d, 0) > 0
        THEN ROUND((sales.sales_30d::numeric / stk.days_with_stock_30d) * 40, 0)
        -- P2 Baja confianza: 90d ajustada → horizonte 30d
        WHEN COALESCE(stk.days_with_stock_90d, 0) >= 5 AND COALESCE(s90.sales_90d, 0) > 0
        THEN ROUND((s90.sales_90d::numeric / stk.days_with_stock_90d) * 30, 0)
        -- P3 Baja confianza: 30d simple → horizonte 30d
        WHEN COALESCE(sales.sales_30d, 0) > 0
        THEN ROUND((sales.sales_30d::numeric / 30) * 30, 0)
        ...
        ELSE 0
      END as projected_demand_variable,

      -- Razón con horizonte y decaimiento
      CASE
        WHEN COALESCE(stk.days_with_stock_30d, 0) >= 15 AND COALESCE(sales.sales_30d, 0) > 0
        THEN 'Vel. 30d ajustada (' || stk.days_with_stock_30d || 'd con stock, horizonte 55d)'
        WHEN COALESCE(stk.days_with_stock_30d, 0) >= 5 AND COALESCE(sales.sales_30d, 0) > 0
        THEN 'Vel. 30d ajustada (' || stk.days_with_stock_30d || 'd con stock, horizonte 40d)'
        WHEN COALESCE(stk.days_with_stock_90d, 0) >= 5 AND COALESCE(s90.sales_90d, 0) > 0
        THEN 'Vel. 90d ajustada (' || stk.days_with_stock_90d || 'd con stock, horizonte 30d)'
        WHEN COALESCE(sales.sales_30d, 0) > 0
  ...
  -- Guardar velocidad para uso futuro (solo datos reales, no fallback histórico)
...
-- Recrear vista (misma estructura, sin columnas nuevas)
