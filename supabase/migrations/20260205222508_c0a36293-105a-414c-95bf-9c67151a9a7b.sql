-- =====================================================
-- FIX: Corregir cálculos de Reposición IA
-- =====================================================

-- 1. Agregar columna in_transit a la tabla inventory_replenishment
ALTER TABLE inventory_replenishment 
ADD COLUMN IF NOT EXISTS in_transit integer DEFAULT 0;

-- 2. Actualizar vista v_replenishment_details para incluir in_transit
...
-- 3. Recrear función refresh_inventory_replenishment con lógica corregida
...
BEGIN
  ...
  -- CORREGIDO: Órdenes pendientes (excluye cancelled y completed)
  pending_orders AS (
    SELECT 
  ...
  -- CORREGIDO: Entregas usando quantity_delivered + cálculo de in_transit
  delivery_data AS (
    SELECT 
      oi.product_variant_id,
  ...
  SELECT 
    ...
    -- CORREGIDO: Sugerencia descuenta stock + pendientes + en tránsito
