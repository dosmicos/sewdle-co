-- Migración de variantes duplicadas de Sleeping Walker Tigres TOG 0.5
-- Transferir órdenes y entregas de SKUs incorrectos (SHOPIFY-) a SKUs correctos (numéricos)

-- Función para migrar variantes duplicadas de Sleeping Walker Tigres
CREATE OR REPLACE FUNCTION migrate_sleeping_walker_tigres_variants()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  -- Buscar el producto "Sleeping Walker Tigres TOG 0.5"
  SELECT id, name INTO product_record
  FROM public.products 
  WHERE LOWER(name) LIKE '%sleeping%walker%tigres%' 
  ...
  IF product_record.id IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Producto Sleeping Walker Tigres TOG 0.5 no encontrado'
    );
  END IF;
...
SELECT migrate_sleeping_walker_tigres_variants();