-- Migración de variantes duplicadas de Ruana Mapache
-- Transferir órdenes y entregas de SKUs incorrectos (SHOPIFY-) a SKUs correctos (numéricos)

-- Función para migrar variantes duplicadas
CREATE OR REPLACE FUNCTION migrate_ruana_mapache_variants()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  -- Buscar el producto "Ruana Mapache"
  SELECT id, name INTO product_record
  FROM public.products 
  WHERE LOWER(name) LIKE '%ruana%mapache%' 
  LIMIT 1;
  
  IF product_record.id IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Producto Ruana Mapache no encontrado'
    );
  END IF;
...
SELECT migrate_ruana_mapache_variants();