-- Fix sales velocity calculation to include all delivered statuses
...
BEGIN
  ...
  LOOP
    ...
    -- FIXED: Include approved quantities from all delivery statuses that represent real sales
    SELECT COALESCE(
      SUM(di.quantity_approved)::NUMERIC / GREATEST(30, 1), 
      0
    ) INTO velocity
    ...
    AND d.status IN ('approved', 'partial_approved')  -- Include both approved and partial approved
    AND di.quantity_approved > 0;  -- Only count items that were actually approved
