-- Actualizar la función calculate_replenishment_suggestions para usar datos de ventas de Shopify
...
BEGIN
  ...
  LOOP
    -- CAMBIO CRÍTICO: Calcular velocidad de ventas usando datos reales de Shopify (últimos 30 días)
    ...
    -- Calcular órdenes abiertas (PRODUCCIÓN - esto sí es correcto usar order_items)
    ...
    IF days_stock <= 7 THEN
      urgency := 'critical';
      reason_text := format('Stock crítico: solo %s días de inventario (ventas Shopify: %s/día)', 
        ROUND(days_stock, 1), ROUND(velocity, 2));
    ELSIF days_stock <= 15 THEN
      urgency := 'high';
      reason_text := format('Stock bajo: %s días de inventario (ventas Shopify: %s/día)', 
        ROUND(days_stock, 1), ROUND(velocity, 2));
    ELSIF suggestion > 0 THEN
      urgency := 'normal';
      reason_text := format('Reposición normal: velocidad ventas %s/día, lead time %s días', 
        ROUND(velocity, 2), total_lead_time);
    ELSE
      urgency := 'low';
      reason_text := format('Stock suficiente: %s días de inventario (ventas Shopify: %s/día)', 
        ROUND(days_stock, 1), ROUND(velocity, 2));
    END IF;
