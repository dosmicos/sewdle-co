-- Fix inconsistent consumption records where workshop_id doesn't match order assignment
-- First, let's update existing consumption records to use the correct workshop from order assignment

UPDATE material_deliveries 
SET workshop_id = wa.workshop_id,
    updated_at = now()
FROM workshop_assignments wa
WHERE material_deliveries.order_id = wa.order_id 
  AND material_deliveries.quantity_consumed > 0
  AND material_deliveries.workshop_id != wa.workshop_id;

-- Update the consume_order_materials function to ensure it uses the correct workshop
