-- Corregir nombre de tabla Shopify: shopify_order_items -> shopify_order_line_items
...
BEGIN
  ...
  WITH sales_data AS (
    ...
    JOIN shopify_order_line_items soi ON so.id = soi.shopify_order_id
  ...
  WHERE p.organization_id = org_id
    AND (
      COALESCE(sd.sales_30d, 0) > 0 
      OR COALESCE(pv.stock_quantity, 0) > 0
    );
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Error en refresh_inventory_replenishment: % %', SQLERRM, SQLSTATE;
    RETURN jsonb_build_object(
      'success', false,
      'error', SQLERRM,
      'sqlstate', SQLSTATE
...
-- Recrear vista con información completa de reposición
...
ORDER BY 
  CASE ir.urgency
    ...
    ELSE 5
  END,
  ir.days_of_supply ASC NULLS LAST;