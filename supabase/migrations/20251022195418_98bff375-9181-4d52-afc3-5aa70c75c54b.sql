-- Fix pending production calculation to use quantity_approved instead of quantity_delivered
...
BEGIN
  ...
  SELECT
    ...
    -- CORRECTED: Pending production = ordered - approved (not delivered)
    COALESCE(
    ...
    -- CORRECTED: Suggested quantity using approved quantities (not delivered)
    GREATEST(
      0,
      ROUND(
        COALESCE(
