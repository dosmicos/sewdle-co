-- Renombrar columnas del CTE para evitar ambigüedad con columnas de destino
...
BEGIN
  ...
  -- Insertar nuevas sugerencias con alias únicos en el CTE
  ...
  WITH calculations AS (
    SELECT 
      pv.id AS variant_id,
      pv.stock_quantity AS calc_current_stock,
  ...
  SELECT 
    ...
    GREATEST(
      0,
      CEIL(c.calc_projected_demand + c.calc_open_orders - c.calc_current_stock)
    )::INTEGER AS suggested_qty,
    
    -- Nivel de urgencia
    CASE 
      WHEN c.calc_days_of_stock < 7 THEN 'critical'
      WHEN c.calc_days_of_stock < 14 THEN 'high'
      WHEN c.calc_days_of_stock < 30 THEN 'normal'
      ELSE 'low'
    END AS urgency,
    
    -- Razón
    CASE 
      WHEN c.calc_days_of_stock < 7 THEN 'Stock crítico: menos de 7 días de inventario'
      WHEN c.calc_days_of_stock < 14 THEN 'Stock bajo: menos de 14 días de inventario'
      WHEN c.calc_days_of_stock < 30 THEN 'Reposición normal: menos de 30 días de inventario'
  ...
  FROM calculations c
  WHERE GREATEST(0, CEIL(c.calc_projected_demand + c.calc_open_orders - c.calc_current_stock)) > 0;
