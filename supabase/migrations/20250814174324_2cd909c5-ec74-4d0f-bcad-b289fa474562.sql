...
-- Only admins and designers can access full customer PII
CREATE POLICY "Admin users can view full shopify orders with customer data" 
ON public.shopify_orders 
FOR SELECT 
USING (
  (organization_id = get_current_organization_safe()) 
  AND (get_current_user_role_safe() = ANY (ARRAY['Administrador'::text, 'Diseñador'::text]))
);
...
SELECT 
  ...
  CASE 
    WHEN get_current_user_role_safe() = ANY (ARRAY['Administrador'::text, 'Diseñador'::text]) 
    THEN customer_email 
...
ALTER VIEW public.shopify_orders_summary SET (security_barrier = true);

-- Create policy for the summary view that allows all authenticated users in organization
CREATE POLICY "Users can view sanitized shopify orders summary" 
ON public.shopify_orders_summary 
FOR SELECT 
USING (
  (organization_id = get_current_organization_safe()) 
