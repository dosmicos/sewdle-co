-- Drop and recreate function to include pending orders in replenishment calculations
...
BEGIN
  ...
  -- NOW INCLUDING PENDING ORDERS IN SALES CALCULATION
  WITH sales_data AS (
    SELECT 
      pv.id AS variant_id,
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 AND 
           ROUND((COALESCE(pv.stock_quantity, 0) + COALESCE(pp.pending_qty, 0)) / (sd.sales_30d / 30.0), 1) < 20
