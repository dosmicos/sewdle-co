...
-- Fix days_of_supply to use the same adjusted velocity as avg_daily_sales
...
BEGIN
  ...
  -- Días con stock disponible en cada ventana
  stock_days AS (
    SELECT 
  ...
  -- Última velocidad conocida guardada
  last_velocity AS (
  ...
  replenishment_calc AS (
    SELECT 
      ...
      -- Días de suministro (FIXED: usa la misma velocidad ajustada)
      CASE
