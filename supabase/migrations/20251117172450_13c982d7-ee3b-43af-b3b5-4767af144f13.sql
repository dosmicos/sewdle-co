-- Update function to calculate days_of_supply based only on current_stock
...
BEGIN
  ...
  SELECT 
    ...
    -- days_of_supply now only considers current_stock, not pending_production
    CASE 
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 
      THEN ROUND(COALESCE(pv.stock_quantity, 0) / (sd.sales_30d / 30.0), 1)
      ELSE 999
    END,
    ...
    -- urgency now based on current_stock only
    CASE 
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 AND 
           ROUND(COALESCE(pv.stock_quantity, 0) / (sd.sales_30d / 30.0), 1) < 7
      THEN 'critical'
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 AND 
           ROUND(COALESCE(pv.stock_quantity, 0) / (sd.sales_30d / 30.0), 1) < 14
      THEN 'high'
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 AND 
           ROUND(COALESCE(pv.stock_quantity, 0) / (sd.sales_30d / 30.0), 1) < 21
      THEN 'medium'
      ELSE 'low'
    END,
    -- reason now based on current_stock only
    CASE 
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 AND 
           ROUND(COALESCE(pv.stock_quantity, 0) / (sd.sales_30d / 30.0), 1) < 7
      ...
      WHEN ROUND(sd.sales_30d / 30.0, 2) > 0 AND 
           ROUND(COALESCE(pv.stock_quantity, 0) / (sd.sales_30d / 30.0), 1) < 20
