-- Incluir órdenes pendientes en el cálculo de ventas 30 días
-- Modificar refresh_inventory_replenishment para incluir financial_status = 'pending'
...
BEGIN
  ...
  -- AHORA INCLUYE ÓRDENES PENDIENTES
  WITH sales_data AS (
    SELECT 
    ...
    WHERE so.organization_id = org_id
      ...
      AND so.financial_status IN ('paid', 'partially_paid', 'authorized', 'pending')
