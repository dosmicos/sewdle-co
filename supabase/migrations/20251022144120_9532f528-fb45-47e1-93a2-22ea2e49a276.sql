-- Actualizar cálculos de ventas a 30 días (migración condicional)

-- 1. Renombrar columnas solo si existen con nombres antiguos
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'inventory_replenishment' AND column_name = 'sales_60d'
  ) THEN
    ALTER TABLE inventory_replenishment RENAME COLUMN sales_60d TO sales_30d;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'inventory_replenishment' AND column_name = 'orders_count_60d'
  ) THEN
    ALTER TABLE inventory_replenishment RENAME COLUMN orders_count_60d TO orders_count_30d;
  END IF;
END $$;
