-- Fix pending_production calculation in refresh_inventory_replenishment
-- The previous version had an incorrect JOIN that referenced a non-existent column
...
BEGIN
  -- CTE for pending production quantities from internal orders
  ...
  SELECT 
    ...
    -- Suggested quantity uses (avg_daily_sales * 40) - stock - pending
    GREATEST(
      0,
      ROUND(
        (ROUND(
