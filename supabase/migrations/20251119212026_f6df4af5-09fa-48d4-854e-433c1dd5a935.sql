-- Update refresh_inventory_replenishment to use 60-day projection instead of 40-day
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE NULL
    END,
    ROUND(COALESCE(sd.sales_30d, 0) * 60.0 / 30.0, 0),
    CASE
      WHEN COALESCE(pv.stock_quantity, 0) = 0 AND COALESCE(sd.sales_30d, 0) > 0 THEN
        GREATEST(0, ROUND(COALESCE(sd.sales_30d, 0) * 60.0 / 30.0, 0) - (COALESCE(pp.pending_qty, 0) - COALESCE(dq.total_delivered, 0)))
      WHEN COALESCE(sd.sales_30d, 0) > 0 THEN
        GREATEST(0, ROUND(COALESCE(sd.sales_30d, 0) * 60.0 / 30.0, 0) - COALESCE(pv.stock_quantity, 0) - (COALESCE(pp.pending_qty, 0) - COALESCE(dq.total_delivered, 0)))
      ELSE 0
    END,
    CASE
