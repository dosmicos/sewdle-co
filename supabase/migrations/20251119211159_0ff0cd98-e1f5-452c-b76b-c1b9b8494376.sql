-- Update refresh_inventory_replenishment to subtract pending production from suggested quantity
  ...
  SELECT 
    ...
    CASE
      WHEN COALESCE(pv.stock_quantity, 0) = 0 AND COALESCE(sd.sales_30d, 0) > 0 THEN
        GREATEST(0, ROUND(COALESCE(sd.sales_30d, 0) * 40.0 / 30.0, 0) - (COALESCE(pp.pending_qty, 0) - COALESCE(dq.total_delivered, 0)))
      WHEN COALESCE(sd.sales_30d, 0) > 0 THEN
        GREATEST(0, ROUND(COALESCE(sd.sales_30d, 0) * 40.0 / 30.0, 0) - COALESCE(pv.stock_quantity, 0) - (COALESCE(pp.pending_qty, 0) - COALESCE(dq.total_delivered, 0)))
      ELSE 0
    END,
    CASE
