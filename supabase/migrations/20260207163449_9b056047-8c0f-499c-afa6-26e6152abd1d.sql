...
-- Cambiar proyección de demanda de 21 a 30 días
...
BEGIN
  ...
  replenishment_calc AS (
    SELECT 
      ...
      -- Proyección a 30 días (antes 21)
      CASE
        WHEN COALESCE(stk.days_with_stock_30d, 0) >= 5 AND COALESCE(sales.sales_30d, 0) > 0
        THEN ROUND((sales.sales_30d::numeric / stk.days_with_stock_30d) * 30, 0)
        WHEN COALESCE(stk.days_with_stock_90d, 0) >= 5 AND COALESCE(s90.sales_90d, 0) > 0
        THEN ROUND((s90.sales_90d::numeric / stk.days_with_stock_90d) * 30, 0)
        WHEN COALESCE(sales.sales_30d, 0) > 0
        THEN ROUND((sales.sales_30d::numeric / 30) * 30, 0)
        WHEN COALESCE(lv.saved_velocity, 0) > 0
        THEN ROUND(lv.saved_velocity * 30, 0)
        ELSE 0
      END as projected_demand_30d,
      CASE
        WHEN COALESCE(stk.days_with_stock_30d, 0) >= 5 AND COALESCE(sales.sales_30d, 0) > 0 
  ...
  SELECT 
    ...
    rc.avg_daily_sales,
    rc.days_of_supply,
    rc.projected_demand_30d,
    GREATEST(0, rc.projected_demand_30d - rc.current_stock - rc.pending_production - rc.in_transit) as suggested_quantity,
    CASE 
