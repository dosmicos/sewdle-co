...
serve(async (req) => {
  ...
  try {
    ...
    if (!amount || !customerEmail || !organizationId) {
      return new Response(
        JSON.stringify({
          error: "Monto, correo del cliente y organizationId son requeridos",
        }),
        {
          status: 400,
    ...
    let boldApiKey = org?.bold_credentials
      ? ((org.bold_credentials as any).api_key ||
        (org.bold_credentials as any).apiKey)
      : null;
    ...
    if (!boldApiKey) {
      return new Response(
        JSON.stringify({
          error:
            "No se encontró la API key de Bold. Por favor configura las credenciales de Bold.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      "Bold credentials source:",
      org?.bold_credentials ? "database" : "env",
    );

    // Generate unique reference with timestamp to avoid duplicates
    const reference = `order_${Date.now()}_${
    ...
    console.log(
      "Creating Bold payment link for amount:",
      amount,
      "reference:",
      reference,
    );
    ...
    console.log(
      "Bold request prepared:",
      JSON.stringify({
        amount_type: paymentLinkData.amount_type,
        amount: paymentLinkData.amount,
        reference: paymentLinkData.reference,
        description: paymentLinkData.description,
        expiration_date: paymentLinkData.expiration_date,
        payment_methods: paymentLinkData.payment_methods,
        payer_email_present: Boolean(paymentLinkData.payer_email),
      }),
    );
    ...
    if (!boldResponse.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: "Error al crear link de pago en Bold",
          details: errorText,
        }),
        {
    ...
    const paymentLinkId = boldData.payload?.payment_link ||
      boldData.payment_link || boldData.id;
    const paymentUrl = boldData.payload?.url || boldData.url ||
      `https://checkout.bold.co/${paymentLinkId}`;
