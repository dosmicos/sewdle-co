...
Deno.test("messaging-ai-openai prioritizes visible text in screenshots of products", async () => {
  ...
  assert(
    source.includes("Usa solo el OCR/texto visible y lo que escriba el cliente"),
...
Deno.test("messaging-ai-openai uses visual OCR clues to build catalog candidates", async () => {
  const source = await Deno.readTextFile(new URL("./index.ts", import.meta.url));

  assert(
    source.includes("buildProductSearchContext") &&
      source.includes("extractVisualCandidateSearchTerms") &&
      source.includes("buildVisualCandidateInstruction"),
    "messaging-ai-openai must reuse shared product search helpers for visual candidates",
  );
  assert(
    source.includes("messagesForAI") && source.includes("visualCandidateTerms.length > 0"),
    "messaging-ai-openai must enrich image messages before catalog selection and branch on visual candidate terms",
  );
});
